...
winston.add(winston.transports.File, { filename: '/var/lib/cloud9/bonescript.log' });
...
f.getPinMode = function(pin, callback) {
    winston.debug('getPinMode(' + pin.key + '): ' + muxFile);
...
f.pinMode = function(pin, direction, mux, pullup, slew, callback) {
    ...
    try {
        ...
        winston.error('Unable to configure mux for pin ' + pin + ': ' + ex);
        gpio[n] = {};
        if(callback) callback(false);
        return(false);
    }
...
f.getEeproms = function(callback) {
    var EepromFiles = {
        ...
        '/sys/bus/i2c/drivers/at24/1-0054/eeprom': { type: 'cape' },
        '/sys/bus/i2c/drivers/at24/1-0055/eeprom': { type: 'cape' },
        '/sys/bus/i2c/drivers/at24/1-0056/eeprom': { type: 'cape' },
        '/sys/bus/i2c/drivers/at24/1-0057/eeprom': { type: 'cape' }
    };
...
f.addLoop = function(loopFunc, loopDelay, callback) {
    ...
    winston.info('Adding loop ' + loopFunc);
    ...
    var message_handler = function(m) {
        winston.info('Parent got message ' + JSON.stringify(m));
        if(m.resolve) {
            var pairs = [];
            for(var name in m.resolve) {
                winston.info('m.resolve[' + name + '] = ' + m.resolve[name]);
...
f.run = function(run_setup, run_loop) {
    ...
    run_loop =
        ...
        [];
    if(cluster.isMaster) {
        //winston.debug('Evaluating ' + run_setup);
        //eval('(' + run_setup + ')();');
        run_setup();
        for(var x in run_loop) f.addLoop(run_loop[x], 0);
        cluster.on('death', function(worker) {
            winston.info('Loop with PID ' + worker.pid + ' died');
        });
    } else {
        var childResolve = function childResolve(varNames) {
            var message = {'resolve': varNames};
            winston.debug('Child sending message ' + JSON.stringify(message));
            process.send(message);
            var myListener = function(m) {
                winston.debug('Child got message ' + JSON.stringify(m));
                if(m.vars) {
                    for(var pair in m.vars) {
                        try {
                            winston.debug(m.vars[pair].name + ' = ' + m.vars[pair].value);
                            myeval(m.vars[pair].name + ' = ' + m.vars[pair].value);
                        } catch(ex) {
                            winston.error('Unable to eval loop in ' + JSON.stringify(m));
                        }
                    }
                    ...
                    var message_handler = function(m) {
                        if(m.readVars) {
                            winston.info('Child got message ' + JSON.stringify(m));
                            var message = {
                                'readVars': true,
                                'loopFunc': loopFunc,
                                'loopDelay': loopDelay
...
lowByte = exports.lowByte;
highByte = exports.highByte;
bitRead = exports.bitRead;
bitWrite = exports.bitWrite;
bitSet = exports.bitSet;
bitClear = exports.bitClear;
bit = exports.bit;
sin = exports.sin;
cos = exports.cos;
tan = exports.tan;
...
max = exports.max;
abs = exports.abs;
constrain = exports.constrain;
map = exports.map;
pow = exports.pow;
sqrt = exports.sqrt;
randomSeed = exports.randomSeed;
