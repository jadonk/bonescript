...
f.analogRead = function(pin, callback) {
    ...
    pin = getpin(pin);
    if(!this.filePrefix) {
        var slots = file_find('/sys/devices', /^bone_capemgr\./) + '/slots';
        var helper = file_find('/sys/module/bone_iio_helper/drivers/platform:bone-iio-helper', /^helper\./);
        var prefix_new = helper + '/AIN';
        var prefix_old = '/sys/bus/platform/devices/tsc/ain';
        if(file_existsSync(slots)) {
            if(!file_existsSync(prefix_new + '0')) {
                fs.writeFileSync(slots, 'cape-bone-iio');
                for(var i = 0; i < 10000; i++) {
                    if(file_existsSync(prefix_new + '0')) break;
                }
            }
        }
        if(file_existsSync(prefix_new + '0')) {
    ...
    var ainFile = this.filePrefix + (pin.ain + this.indexOffset).toString();
    if(callback) {
        var readFile = function(err, data) {
            if(err) {
...
f.getEeproms = function(callback) {
    var eeproms = {};
    var capemgr = file_find('/sys/devices', /^bone_capemgr./);
...
f.getPlatform = function(callback) {
    ...
    var capemgr = file_find('/sys/devices', /^bone_capemgr./);
    if(file_existsSync(capemgr + '/baseboard/board-name')) {
