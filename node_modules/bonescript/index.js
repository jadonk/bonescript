...
try {
    ...
    if(my.is_capemgr()) {
        ...
        // Need to add a test here that is valid on old kernel
    }
} catch(ex) {
    hw = hw_simulator;
...
f.getPinMode = function(pin, callback) {
    ...
    // Get pinmux settings
    var mode = hw.readPinMux(pin, mode, callback);
    return(mode);
};
...
f.pinMode = function(pin, direction, mux, pullup, slew, callback) {
    ...
    if(mux == 7) {
        if(typeof gpio[n] == 'undefined') {
            ...
            resp = hw.exportGPIOControls(n, resp);
            if(typeof resp.err != 'undefined') {
                delete gpio[n];
                if(callback) callback(resp);
                return(resp.value);
...
f.attachInterrupt = function(pin, handler, mode, callback) {
    ...
    if(typeof gpio[pin.gpio] == 'undefined') {
        resp.attached = false;
    ...
    try {
        ...
        gpioInt[n].epoll.valuefd = valuefd;
        resp.attached = true;
    } catch(ex) {
...
f.detachInterrupt = function(pin, callback) {
    ...
    gpioInt[n].epoll.remove(gpio[pin.gpio].epoll.valuefd);
    delete gpioInt[pin.gpio];
