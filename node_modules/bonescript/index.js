...
f.analogRead = function(pin, callback) {
    ...
    if(typeof this.ainPrefix == 'undefined') {
        if(load_dt('cape-bone-iio')) {
            var ocp = file_find('/sys/devices', 'ocp.', 1000);
            var helper = file_find(ocp, 'helper.', 10000)
...
var serialport = myrequire('serialport');
var serialPorts = {};
f.serialOpen = function(port, options, callback) {
...
f.serialOpen.args = ['port', 'options', 'callback'];

f.serialWrite = function(port, data, callback) {
    if(!serialport.exists) {
        if(callback) callback({err:'serialport module not loaded'});
        return(false);
    }
...
f.serialWrite.args = ['port', 'data', 'callback'];
