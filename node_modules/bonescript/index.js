...
getLoops = exports.getLoops = function(callback) {
    ...
    for(var worker in myWorkers) {
        var loop = {};
        loop.id = myWorkers[worker].worker.pid;
        loop.loopFunc = myWorkers[worker].loopFunc;
        loop.loopDelay = myWorkers[worker].loopDelay;
        loops.push(loop);
    }
    console.log('loops = ' + JSON.stringify(loops));
    if(callback) {
