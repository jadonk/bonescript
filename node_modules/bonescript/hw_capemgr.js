...
exports.setPinMode = function(pin, pinData, template, resp) {
    ...
    if(template == 'bspwm') {
        ...
        pwmPrefix[pin.key] = pwm_test;
        fs.writeFileSync(pwm_test+'/polarity', 0);
    } else {
        resp.err = 'Error enabling PWM controls';
    }
    return(resp);
};
...
exports.exportGPIOControls = function(n, resp) {
...
exports.readAIN = function(pin, resp, callback) {
    ...
    if(isNaN(resp.value)) {
        resp.err = 'analogRead(' + pin.key + ') returned ' + data;
        winston.error(resp.err);
    }
    resp.value = resp.value / 1800;
    if(isNaN(resp.value)) {
        resp.err = 'analogRead(' + pin.key + ') scaled to ' + data;
    }
    return(resp);
};
