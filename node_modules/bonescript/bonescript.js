        ...
        for(var x in m.data) {
            ...
            if(m.data[x].type == 'function') {
                ...
                var handyfunc = '_bonescript.modules["' + m.module + '"].' + m.data[x].name +
                    ...
                    ' if(callback) {\n' +
                    '  _bonescript._seqnum++;\n' +
                    '  callbacks[_bonescript._seqnum] = callback;\n' +
                    '  calldata.seq = seqnum;\n' +
                    ' }\n' +
                    ' socket.emit("' +
                    m.module + '$' + m.data[x].name +
                    '", calldata);\n' +
                    '};\n';
                eval(handyfunc);
            } else {
                _bonescript.modules[m.data[x].name] = m.data[x].value;
            }
        }
    }
    function seqcall(data) {
        if(data.seq && (typeof _bonescript._callbacks[data.seq] == 'function')) {
...
(function () {
    ...
    var scriptObj = head.appendChild('/socket.io/socket.io.js');
    scriptObj.onload = bonescript._onSocketIOLoaded;
})();
