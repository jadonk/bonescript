...
exports.wrapOpen = function(m) {
    if(!m.module.exists) {
        winston.info(m.name + ' support module not loaded.');
        return(function(){});
    }
    var newFunction = function(port, options, callback) {
        ...
        m.openPorts[port] = new m.openMethod(port, options);
        if(!m.openPorts[port]) {
    ...
    newFunction.args = ['port', 'options', 'callback'];
    return(newFunction);
};
