...
module.exports = {
	enable : function(path, callback){
		...
		bone.find_sysfsFile(sclPin + "_pinmux", bone.is_ocp(), p + '.', onFindPinmux);
		...
		function onWriteState(err) {
			...
			if (setSclPinState) {
				...
				setSclPinState = true;
				bone.find_sysfsFile(sdaPin + "_pinmux", bone.is_ocp(), p + '.', onFindPinmux);
			}
		}
	},

	open : function(port, address, handler, callback) {
		...
		function onI2CEnable() {
			if (!i2c.openPorts[path]) {
				var port = new i2cPort(address , { device : i2c.ports[path].path });
