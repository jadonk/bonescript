...
module.exports = {
	...
	enable : function(path, callback) {
		...
		bone.find_sysfsFile(txPin + "_pinmux", bone.is_ocp(), p + '.', onFindPinmux);
		...
		function onWriteState(err) {
			...
			if (setTxPinState) {
				...
				setTxPinState = true;
				bone.find_sysfsFile(rxPin + "_pinmux", bone.is_ocp(), p + '.', onFindPinmux);
			}
		}
	},
