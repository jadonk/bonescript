...
f.pinMode = function(pin, direction, mux, pullup, slew, callback) {
    ...
    if(typeof resp.err != 'undefined') {
        ...
        // Don't exit yet, it might work if the pin is already muxed to desired mode
        if(debug) winston.debug('pinMode: mode = ' + JSON.stringify(f.getPinMode(pin)));
...
f.digitalRead = function(pin, callback) {
    ...
    if(typeof pin.ain != 'undefined') {
        if(callback) {
            resp = f.analogRead(pin, analogCallback);
        } else {
            resp = analogValue(f.analogRead(pin));
        }
    } else {
...
f.analogRead = function(pin, callback) {
    ...
    var resp = {};
    if(typeof pin.ain == 'undefined') {
        resp = f.digitalRead(pin, callback);
    } else {
        if(!ain) {
            ain = hw.enableAIN();
        }
